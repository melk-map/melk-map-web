...
import { type GeoJSONProperties, GeoJSONPropertiesSchema } from "./properties";
...
>(
	...
	z
		.looseObject({
			...
			coordinates: z
				.never({
					error: "GeoJSON feature collection cannot have a 'coordinates' key",
				})
				.optional(),
			geometry: z
				.never({
					error: "GeoJSON feature collection cannot have a 'geometry' key",
				})
				.optional(),
			properties: z
				.never({
					error: "GeoJSON feature collection cannot have a 'properties' key",
				})
				.optional(),
			geometries: z
				.never({
					error: "GeoJSON feature collection cannot have a 'geometries' key",
				})
				.optional(),
		})
		.check((ctx) => {
			...
			if (!validDimensionsForFeatureCollection(ctx.value)) {
				ctx.issues.push(
					getInvalidFeatureCollectionDimensionsIssue(ctx),
				);
				return;
			}
...
export const GeoJSON2DFeatureCollectionSchema =
	GeoJSONFeatureCollectionGenericSchema(
		GeoJSON2DPositionSchema,
		GeoJSONPropertiesSchema.nullable(),
		GeoJSON2DGeometrySchema,
	);
export type GeoJSON2DFeatureCollection = z.infer<
	typeof GeoJSON2DFeatureCollectionSchema
>;

export const GeoJSON3DFeatureCollectionSchema =
	GeoJSONFeatureCollectionGenericSchema(
		GeoJSON3DPositionSchema,
		GeoJSONPropertiesSchema.nullable(),
		GeoJSON3DGeometrySchema,
	);
export type GeoJSON3DFeatureCollection = z.infer<
	typeof GeoJSON3DFeatureCollectionSchema
>;
