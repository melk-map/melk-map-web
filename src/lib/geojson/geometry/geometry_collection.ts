...
import { GeoJSONBaseSchema, type GeoJSONBaseSchemaShape } from "../base";
import {
...
import {
	getInvalidGeometryCollectionDimensionIssue,
	validDimensionsForCollection,
} from "./validation/dimension";

export type GeoJSONGeometryCollectionGenericSchemaType<
...
export const GeoJSONGeometryCollectionGenericSchema = <
	P extends GeoJSONAnyPosition,
>(
	...
	z
		.looseObject({
			...
			coordinates: z
				.never({
					error: "GeoJSON geometry collection cannot have a 'coordinates' key",
				})
				.optional(),
			geometry: z
				.never({
					error: "GeoJSON geometry collection cannot have a 'geometry' key",
				})
				.optional(),
			properties: z
				.never({
					error: "GeoJSON geometry collection cannot have a 'properties' key",
				})
				.optional(),
			features: z
				.never({
					error: "GeoJSON geometry collection cannot have a 'features' key",
				})
				.optional(),
		...
		.check((ctx) => {
			...
			if (!validDimensionsForCollection(ctx.value)) {
				ctx.issues.push(
					getInvalidGeometryCollectionDimensionIssue(ctx),
				);
			}
...
export const GeoJSONGeometryCollectionSchema =
	GeoJSONGeometryCollectionGenericSchema(GeoJSONPositionSchema);
export type GeoJSONGeometryCollection = z.infer<
	typeof GeoJSONGeometryCollectionSchema
>;

export const GeoJSON2DGeometryCollectionSchema =
	GeoJSONGeometryCollectionGenericSchema(GeoJSON2DPositionSchema);
export type GeoJSON2DGeometryCollection = z.infer<
	typeof GeoJSON2DGeometryCollectionSchema
>;

export const GeoJSON3DGeometryCollectionSchema =
	GeoJSONGeometryCollectionGenericSchema(GeoJSON3DPositionSchema);
export type GeoJSON3DGeometryCollection = z.infer<
	typeof GeoJSON3DGeometryCollectionSchema
>;
