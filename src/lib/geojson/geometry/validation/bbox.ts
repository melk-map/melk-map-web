...
export const getInvalidBBoxIssue = (
 ...
 */
export function validBBoxForPosition({
	bbox,
	coordinates,
}: ValidatableCoordinate): boolean {
	...
	return bbox.every(
		(value, index) => value === coordinates[index % dimension],
	);
}
 ...
 */
export function validBBoxForPositionList({
	bbox,
	coordinates,
}: ValidatableList): boolean {
	if (bbox == null) return true;
	if (coordinates == null) return false;
 ...
 */
export function validBBoxForPositionGrid({
	bbox,
	coordinates,
}: ValidatableGrid): boolean {
	if (bbox == null) return true;
	if (coordinates == null) return false;
 ...
 * @param coordinates Contains the grid of positions
 */
export function validBBoxForPositionGridList({
	bbox,
	coordinates,
}: ValidatableGridList): boolean {
	if (bbox == null) return true;
	if (coordinates == null) return false;
...
export function validBBoxForCollection({
	bbox,
	geometries,
...
export function getBBoxForGeometries(
...
function updateBBoxForPositionGridList(
	...
	positions?.forEach((positionGrid) =>
		updateBBoxForPositionGrid(currentBBox, positionGrid),
	);
	return currentBBox;
}
...
function updateBBoxForPositionGrid(
	...
	positions?.forEach((positionList) =>
		updateBBoxForPositionList(currentBBox, positionList),
	);
	return currentBBox;
}
...
function updateBBoxForPositionList(
	...
	positions?.forEach((position) =>
		updateBBoxForPosition(currentBBox, position),
	);
	return currentBBox;
}
 ...
 * NOTE: Mutates the given bbox. Performance optimisation to avoid unnecessary copies.
 */
function updateBBoxForPosition(
	currentBBox: number[],
...
function mergeBBoxs(bboxs: number[][]): number[] {
	...
	for (let i = 0; i < dimension; i++) {
		...
		mergedBBox[i + dimension] = Math.max(
			...bboxs.map((bbox) => bbox[i + dimension]),
		);
	}
	return mergedBBox;
}
